...
class API extends REST {
    ...
    const DB_PASSWORD_PROD = "Emsi1234";
    const DB_PROD = "citiesvivbroot";
    const DB_PREFIX="casablanca_";
   
    const DB_SERVER_LOCAL = "localhost";
    const DB_USER_LOCAL = "root";
    const DB_PASSWORD_LOCAL = "admin";
    const DB_LOCAL = "the_city";
    
    const GOOGLE_API_KEY = "AAAAcrN_TBw:APA91bHU6in0Repy0HlHn-LbCgA6dCADq2KsjGdm4mv3jVxzJolsiB0AakdjOgkoo4dcasb8sVoF55_1CvMVIG43E_xprZ1OPw8YD-oniZexuTPWQpYgFf6w4LHeEmnAOhWPHBBD9vwA";
    ...
    private function dbConnect() {
        if (self::local_version)
           $this->mysqli = new mysqli(self::DB_SERVER_LOCAL, self::DB_USER_LOCAL, self::DB_PASSWORD_LOCAL, self::DB_LOCAL);
            ...
            $this->mysqli = new mysqli(self::DB_SERVER_PROD, self::DB_USER_PROD, self::DB_PASSWORD_PROD, self::DB_PROD);
    ...
    private function listPlaces() {
        ...
        foreach ($places as $r) {
            ...
            if ($draft != 1)
                $r["images"] = $this->getImagesArrayByPlaceId($r["place_id"]);
    ...
    private function sendNotif() {
        ...
        foreach ($pushStatus as $s) {
            if (!empty($s['success']))
                $success_count = $success_count + $s['success'];
            if (!empty($s['failure']))
                $failure_count = $failure_count + $s['failure'];
        }
